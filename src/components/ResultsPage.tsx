...
export function ResultsPage({ labConfig, patientId, files, isLoading }: ResultsPageProps) {
  ...
  const handleViewAll = useCallback(async () => {
    try { await logAccess(labConfig.labKey, patientId, 'ALL', 'VIEW_ALL'); } catch {}
    if (hasMixed) {
  ...
  const handleViewImages = useCallback(async () => {
    try { await logAccess(labConfig.labKey, patientId, 'ALL', 'VIEW_ALL_IMAGES'); } catch {}
    setSelectedIndex(0);
    setViewerMode('images');
    imageFiles.forEach(f => logAccess(labConfig.labKey, patientId, f.id, 'view'));
  }, [imageFiles, labConfig.labKey, patientId]);

  const handleViewPdfs = useCallback(async () => {
    try { await logAccess(labConfig.labKey, patientId, 'ALL', 'VIEW_ALL_PDFS'); } catch {}
    setViewerMode('pdfs');
    pdfFiles.forEach(f => logAccess(labConfig.labKey, patientId, f.id, 'view'));
  }, [pdfFiles, labConfig.labKey, patientId]);

  const handleDownloadAll = useCallback(async () => {
    // Log "download all" intent (and also each file later)
    try {
      await logAccess(labConfig.labKey, patientId, 'ALL', 'DOWNLOAD_ALL');
    } catch {}
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        // Log as if viewed/downloaded
        await logAccess(labConfig.labKey, patientId, file.id, 'DOWNLOAD');

        // Direct download (no ZIP) - best for mobile
      ...
      setTimeout(() => setDownloadState(prev => ({ ...prev, show: false })), 2500);
    } catch (error) {
